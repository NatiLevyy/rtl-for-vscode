# תמיכת RTL ל-GitHub Copilot Chat

תוסף שמאפשר כתיבה בעברית (וערבית) ב-GitHub Copilot Chat עם יישור נכון מימין לשמאל.
...
1. לחץ קליק ימני על `install.bat`
...
4. **מזריק RTL** לכל גרסאות GitHub Copilot Chat

## אם RTL מפסיק לעבוד

אם עדכנת את Copilot Chat ו-RTL הפסיק לעבוד, פשוט הרץ:
```powershell
...
| קובץ | תיאור |
|------|-------|
| `install.bat` | לחיצה כפולה להתקנה |
| `install.ps1` | סקריפט התקנה ראשי |
| `copilot-rtl-prepend.js` | קוד RTL ל-Copilot Chat |
...
בדוק את קובץ הלוג:
```
C:\Users\שם_משתמש\vscode-custom\rtl-copilot-inject.log
```

## הסרה

אם תרצה להסיר את RTL:
1. מחק את הקבצים `copilot-rtl-prepend.js` ו-`rtl-auto-inject.ps1` מתיקיית `C:\Users\שם_משתמש\vscode-custom\`
2. מחק את `rtl-copilot-injector.vbs` מתיקיית Startup
3. הרץ `regedit` ומחק את `RTLCopilotInjector` מתחת ל:
   `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`

## קרדיט

מבוסס על [vscode-copilot-rtl](https://github.com/NabiKAZ/vscode-copilot-rtl) עם שיפורים להתקנה אוטומטית.
