/** RTL Support for Google Antigravity - Prepend Version */
;(function(){
    if(window._rtlAntigravityInjected)return;
    window._rtlAntigravityInjected=true;
    ...
    const process=()=>{
        ...
        // Process prose containers (Antigravity specific)
        document.querySelectorAll('.prose, .prose-sm, .whitespace-pre-wrap').forEach(el=>{
            if(isRTL(el.textContent)&&!el.getAttribute('data-rtl'))applyRTL(el);
        });
    ...
    const init=()=>{
        ...
        console.log('✅ RTL Antigravity Active');
    };
