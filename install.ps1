# ===========================================
# RTL Support Installer for Claude Code & Antigravity
# ===========================================
# Run this script as:
...
Write-Host "   For Claude Code & Google Antigravity" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
$FilesToCopy = @(
    "claude-code-rtl-prepend.js",
    "antigravity-rtl-prepend.js",
    "rtl-auto-inject.ps1",
    "rtl-auto-inject.vbs"
)
...
$StartupScript = Join-Path $StartupFolder "rtl-injector.vbs"
...
try {
    New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run' -Name 'RTLInjector' -Value "wscript.exe `"$TargetDir\rtl-auto-inject.vbs`"" -PropertyType String -Force | Out-Null
...
$InjectScript = Join-Path $TargetDir "rtl-auto-inject.ps1"
if (Test-Path $InjectScript) {
    & powershell -ExecutionPolicy Bypass -File $InjectScript
...
Write-Host "  3. Open Claude Code and write in Hebrew!" -ForegroundColor White
Write-Host ""
